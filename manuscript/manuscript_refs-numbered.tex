...
\usepackage{geometry}
\geometry{textwidth=18cm, textheight=23.4cm,marginratio={1:1,5:7}}
...
\hypersetup{
    colorlinks,
    linkcolor={red!50!black},                           % Color of within-text links (e.g., to a Fig.)
...
\usepackage[style=nature,doi=false,url=false,eprint=false]{biblatex}
...
\author[1,2,3,*]{\small Felix Molter}
\author[2,4]{\small Armin W. Thomas}
\author[5,6]{\small Scott A. Huettel}
\author[2,7]{\small Hauke R. Heekeren}
\author[1,2,3]{\small Peter N. C. Mohr}
...
\affil[3]{\footnotesize WZB Berlin Social Science Center, Berlin, Germany}
\affil[4]{\footnotesize Department of Electrical Engineering and Computer Science, Technische Universität Berlin, Berlin, Germany}
\affil[5]{\footnotesize Center for Cognitive Neuroscience, Duke University, Durham, NC, USA}
\affil[6]{\footnotesize Department for Psychology and Neuroscience, Duke University, Durham, NC, USA}
\affil[7]{\footnotesize Department for Education and Psychology, Freie Universität Berlin, Berlin, Germany}
...
% Fig. 1: Choice task and attribute space
\begin{SCfigure}[0.8]
\centering
...
% Fig. 2: Context effects in choices and relative dwell times
\begin{figure}[bt!]
\begin{centering}
...
To better understand participants' eye movements over the course of the trial, we performed additional exploratory analyses of gaze location and transitions. We found an increasing association between gaze and choice over the trial, and longer gaze towards targets, even accounting for choice. Information search occurred both within and between alternatives, with slightly more transitions within alternatives. We refer to \nameref{sup:eye-movement-analyses} for additional details on these analyses.

\subsection*{Behavioural modelling and model comparison}

% Fig. 3: Model comparison results, predicted gaze-choice association RST prediction of winning model
\begin{figure}[bt!]
\begin{centering}
...
\begin{refsection}  % Separate reference list for Supplement

% Reset page counter and add "S"
\setcounter{page}{1}
\renewcommand{\thepage}{S\arabic{page}}

% Reset figure and table counters, change figure and table names
% \renewcommand{\thefigure}{S\arabic{figure}}  % -> Fig. S1
\renewcommand{\figurename}{Supplementary Fig.} % -> Supplementary Fig. 1
\setcounter{figure}{0} 

% \renewcommand{\thetable}{S\arabic{table}}  % -> Table S1
\renewcommand{\tablename}{Supplementary Table} % Supplementary Table 1
\setcounter{table}{0} 

\subsection*{Supplementary Note 1}
\label{sup:eye-movement-analyses}

% Supplementary Fig.: AOI dwell over time
\begin{figure}[ht!]
\begin{centering}
\includegraphics[width=\textwidth]{../figures/S_gaze-data_timebinned_dwell.pdf}
...
% Supplementary Fig.: Dwell-regression weights
...
% Supplementary Fig.: GLA estimates and correlations
\begin{figure}[ht!]
\begin{centering}
\includegraphics[width=\textwidth]{../figures/S_GLA-estimates.pdf}
...
% Supplementary Fig.: Model-predicted choice probabilities
\begin{figure}[ht!]
\begin{centering}
...
% Supplementary Fig.: Relative model fit and RST
\begin{figure}[ht!]
\begin{centering}
...
\textbf{Attribute integration} &
  \multicolumn{3}{l}{\begin{tabular}[c]{@{}l@{}}$\times$ \textbf{Multiplicative}\\ Outcomes and probabilities\\ combine multiplicatively\\ into expected utilities \autocite{tversky1992AdvancesProspectTheory,glickman2019FormationPreferenceRisky}.\end{tabular}} &
  \multicolumn{3}{l}{\begin{tabular}[c]{@{}l@{}}+ \textbf{Weighted additive}\\ Outcomes and probabilities\\ are normalized, weighted\\ and added \autocite{rouault2019PrefrontalMechanismsCombining}.\end{tabular}} &
  \textit{n.a.} &
  2 \\ \midrule
\textbf{Comparison} &
  \multicolumn{3}{l}{\begin{tabular}[c]{@{}l@{}}\textbf{Independent}\\ Accumulation of absolute\\ item values per alternative \autocite{glickman2019FormationPreferenceRisky,bhatia2013AssociationsAccumulationPreference}.\end{tabular}} &
  \multicolumn{3}{l}{\begin{tabular}[c]{@{}l@{}}\textbf{Comparative}\\ Accumulation of relative\\ values per alternative \autocite{roe2001MultialternativeDecisionField}.\end{tabular}} &
  \textit{n.a.} &
  2 \\ \midrule
\textbf{Alternative-wise gaze-discount} &
  ...
  \cellcolor[HTML]{DAE8FC}\begin{tabular}[c]{@{}l@{}}\textbf{True}\\ Non-fixated alternatives' values are\\ discounted by a parameter $\theta$ \autocite{krajbich2010VisualFixationsComputation}.\end{tabular} &
  2 \\ \midrule
\textbf{Attribute-wise gaze-discount} &
  ...
  \cellcolor[HTML]{DAE8FC}\begin{tabular}[c]{@{}l@{}}\textbf{True}\\ Attributes on the non-fixated dimension\\ are discounted by a parameter $\eta$ \autocite{krajbich2012AttentionalDriftdiffusionModel,fisher2017AttentionalDriftDiffusion}.\end{tabular} &
  2 \\ \midrule
\textbf{Accumulation leak} &
  \multicolumn{3}{l}{\begin{tabular}[c]{@{}l@{}}\textbf{None}\\ Perfect integration over fixations.\end{tabular}} &
  \multicolumn{3}{l}{\begin{tabular}[c]{@{}l@{}}\textbf{Constant}\\ With each fixation, accumulators leak\\ information proportional to their current value,\\ controlled by parameter $\lambda$ \autocite{roe2001MultialternativeDecisionField,usher2001TimeCoursePerceptual}.\end{tabular}} &
  \cellcolor[HTML]{DAE8FC}\begin{tabular}[c]{@{}l@{}}\textbf{Gaze-dependent}\\ With each fixation, accumulators of\\ non-fixated alternatives leak information,\\ as in Constant leak \autocite{ashby2016FindingRightFit}.\end{tabular} &
  3 \\ \midrule
\textbf{Accumulator inhibition} &
  \multicolumn{2}{l}{\begin{tabular}[c]{@{}l@{}}\textbf{None}\\ No inhibition between accumulators.\end{tabular}} &
  \multicolumn{2}{l}{\begin{tabular}[c]{@{}l@{}}\textbf{Constant}\\ Accumulators inhibit each other\\ proportional to their current value,\\ controlled by parameter $\Phi$ \autocite{usher2004LossAversionInhibition}.\end{tabular}} &
  \multicolumn{2}{l}{\begin{tabular}[c]{@{}l@{}}\textbf{Distance-dependent}\\ Inhibition between accumulators\\ depends on pairwise distance\\ between alternatives in attribute\\ space. Parameters $w_d$ and $\Phi$ \autocite{roe2001MultialternativeDecisionField}.\end{tabular}} &
...
% Supplementary Fig.: Individual best fitting switch level counts
\begin{figure}[!ht]
\begin{centering}
...
% Supplementary Fig.: Relative dwell advantage for GLA and hybrid, subgroups (split by attraction effect strength)
\begin{figure}[!ht]
\begin{centering}
...
\renewcommand*{\bibfont}{\small}
\emergencystretch=1em % prevent overfull \hbox for 1.5pt
\printbibliography[title=Supplementary Information References]

\end{refsection} % (separate reference list for Supplement)
\renewcommand{\thepage}{S\arabic{page}}
