...
model_labels = {
    "glickman1layerduration": "GLA$_{dur}$",
    "mdft": "MDFT",
    # "eu": "EU",
    "pt": "PT",
...
def plot_bic_violins(estimates, models, model_labels, ax=None):
    ...
    model_order = (
        estimates.loc[estimates['model'].isin(models)].groupby("model")["bic"]
        .mean()
        .reset_index()
...
def plot_n_best_fitting(individual_best_models, models, model_labels, ax=None):
    """
    Makes a barplot of best fitting models in `individual_best_models`.
    """
    if ax is None:
        ax = plt.gca()
...
axb = plot_n_best_fitting(individual_best_models, models, model_labels, ax=axb)
...
for effect, ax in zip(["attraction", "compromise"], [axi, axj]):
    ax = plot_observed_predicted_rst(
        cs,
        predictions.loc[predictions["model"] == "glickman1layerduration"],
        effect=effect,
...
# Save the figure
plt.savefig(
    join(OUTPUT_DIR, "S_model-comparison_gla-duration.pdf"),
    bbox_inches="tight",
)
