...
gla_estimates = estimates.loc[
    estimates["model"] == "glickman1layernoleak",
    ["subject", "bic", "nll", "alpha", "beta", "gamma", "theta"],
].reset_index(drop=True)

gla_estimates_summary = (
    gla_estimates[["alpha", "beta", "gamma", "theta"]].describe().round(2).T
)
output_file = join(OUTPUT_DIR, "gla-noleak_estimates_summary.csv")
gla_estimates_summary.to_csv(output_file)
print(f"Saved summary of GLA_noleak estimates to '{output_file}'.")
...
for e, effect in enumerate(["attraction", "compromise"]):
    ...
    pred_gla = predicted.loc[predicted["model"] == "glickman1layernoleak"]
...
for e, effect in enumerate(["attraction", "compromise"]):
    rst_obs = calc_rst(observed, effect=effect)
    pred_m = predicted.loc[predicted["model"] == "glickman1layernoleak"]
