...
OUTPUT_DIR = join("..", "results", "S_gla-duration")
...
gla_estimates = estimates.loc[
    estimates["model"] == "glickman1layerduration",
...
output_file = join(OUTPUT_DIR, "gla-duration_estimates_summary.csv")
gla_estimates_summary.to_csv(output_file)
print(f"Saved summary of GLA_duration estimates to '{output_file}'.")
...
for e, effect in enumerate(["attraction", "compromise"]):
    ...
    pred_gla = predicted.loc[predicted["model"] == "glickman1layerduration"]
...
for e, effect in enumerate(["attraction", "compromise"]):
    rst_obs = calc_rst(observed, effect=effect)
    pred_m = predicted.loc[predicted["model"] == "glickman1layerduration"]
