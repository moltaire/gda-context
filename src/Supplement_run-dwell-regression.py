...
def runRegression():
    ...
    OUTPUT_DIR = join(
        RESULTS_DIR, "S_supplemental-gaze-analyses", "dwell-regression" + LABEL
    )
    ...
    if MIXED:
        intercept = "0"
        if not TIMEBINNED:
            random_terms = [f"1|subject"] + [f"{p}|subject" for p in predictors]
        else:  # timebinned
    ...
    for effect in ["attraction", "compromise"]:
        ...
        if os.path.exists(ESTIMATES_FILE) and (not OVERWRITE):
            ...
            plot_trace(model.backend.trace, var_names=predictors)
            plt.savefig(
                os.path.join(
                    OUTPUT_DIR,
                    "traceplots",
                    f"dwell-regression{LABEL}_{effect}_traceplot.png",
                ),
                dpi=100,
            )
            plt.close()
